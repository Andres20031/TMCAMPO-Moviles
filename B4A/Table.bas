...
'
'Version 3.24
'Amended problem with JumpToRowAndSelect not beeing selected
'
'Version 3.23
'Amended error when setting RowHeight before the table initialized
'
'Version 3.22
'Amended error with insertRowAt
'
'Version 3.21
'Amended error with TextSize for fixed columns
'
'Version 3.20
...
#DesignerProperty: Key: SingleLine, DisplayName: SinlgleLine, FieldType: Boolean, DefaultValue: True, Description: True = cell text is displayed in a single line; False = text is multiline.
...
#DesignerProperty: Key: RowHeight, DisplayName: RowHeight, FieldType: Int, DefaultValue: 40, Description: Row height.
#DesignerProperty: Key: HeaderHeight, DisplayName: HeaderHeight, FieldType: Int, DefaultValue: 40, Description: Header height.
...
#DesignerProperty: Key: TextSize, DisplayName: TextSize, FieldType: Int, DefaultValue: 14, Description: Text size.
#DesignerProperty: Key: TextAlignment, DisplayName: TextAlignment, FieldType: String, DefaultValue: CENTER, List: LEFT|CENTER|RIGHT, Description: Set the text alignment.
#DesignerProperty: Key: HeaderTextAlignment, DisplayName: HeaderTextAlignment, FieldType: String, DefaultValue: CENTER, List: LEFT|CENTER|RIGHT, Description: Set the header text alignment.
#DesignerProperty: Key: HeaderColor, DisplayName: HeaderColor, FieldType: Color, DefaultValue: 0xFF808080, Description: Header background color.
#DesignerProperty: Key: TableColor, DisplayName: TablColor, FieldType: Color, DefaultValue: 0xFFD3D3D3, Description: Table background color.
#DesignerProperty: Key: HeaderTextColor, DisplayName: HeaderTextColor, FieldType: Color, DefaultValue: 0xFFFFFFFF, Description: Table background color.
#DesignerProperty: Key:	CellTextColor, DisplayName: CellTextColor, FieldType: Color, DefaultValue: 0xFF000000, Description: Table background color.
#DesignerProperty: Key:	Row1Color, DisplayName: Row1Color, FieldType: Color, DefaultValue: 0xFFFFFFFF, Description: Row1 background color.
#DesignerProperty: Key:	Row2Color, DisplayName: Row2Color, FieldType: Color, DefaultValue: 0xFF98F5FF, Description: Row2 background color.
...
#DesignerProperty: Key:	StatusLineHeight, DisplayName: StatusLineHeight, FieldType: Int, DefaultValue: 40, Description: Status line height.
...
#DesignerProperty: Key:	FastScrollLabelMaxChars, DisplayName: FastScrollLabelMaxChars, FieldType: Int, DefaultValue: 10, Description: Max number of characters for the fastscroll Label. Default value = 10.
...
#DesignerProperty: Key:	FastScrollLabelHeight, DisplayName: FastScrollLabelHeight, FieldType: Int, DefaultValue: 30, Description: Height of the fast scroll Label.
#DesignerProperty: Key:	SortCaseInsensitive, DisplayName: SortCaseInsensitive, FieldType: Boolean, DefaultValue: False, Description: Sorts the selected column case insensitive.
#DesignerProperty: Key:	MultiColumnSort, DisplayName: MultiColumnSort, FieldType: Boolean, DefaultValue: False, Description: Allows multi culomn sorting.
...
Sub Class_Globals
	...
	Private Header, HeaderBase As Panel
	...
	Private mFastScrollLabelMaxChars = 10 As Int
	...
	Private cRowHeight, cHeaderColor, cTableColor, cTextColor, cHeaderHeight, cHeaderTextColor, cStatusLineHeight As Int
	...
	Private MultiTextColors = False As Boolean
	...
	Private SelectedDrawable(), Drawable1(), Drawable2(), ColumnDrawables() As Object
	...
	'----------------------------------------------------------
	'RBS added, all the module level, sorting related variables
	'----------------------------------------------------------
	...
	Private mMultiColumnSort As Boolean = False
	Private mSortCaseInsensitive As Boolean = False
...
Public Sub DesignerCreateView (Base As Panel, Lbl As Label, Props As Map)
	...
	cSingleLine = Props.GetDefault("SingleLine", True)
	...
	cStatusLineHeight = DipToCurrent(Props.GetDefault("HeaderHeight", 40))
	...
	mFastScrollLabelMaxChars = Props.GetDefault("FastScrollLabelMaxChars", 10)
	mFastScrollLabelWidth = DipToCurrent(Props.GetDefault("FastScrollLabelWidth", 150dip))
	mFastScrollLabelHeight = DipToCurrent(Props.GetDefault("FastScrollLabelHeight", 30dip))
	mSortCaseInsensitive = Props.GetDefault("SortCaseInsensitive", False)
	mMultiColumnSort = Props.GetDefault("MultiColumnSort", False)
...
Private Sub InitTable
	...
	HeaderBase.Color = Colors.Transparent
	Header.Initialize("")
	...
	If (cShowStatusLine = True) Then
		pnlTable.AddView(SVF, 0, Header.Height, cWidth, cHeight - Header.Height - cStatusLineHeight)
		pnlTable.AddView(SV2, mFirstColumnsWidth, Header.Height, cWidth - mFirstColumnsWidth, cHeight - Header.Height - cStatusLineHeight)
		pnlTable.AddView(lblStatusLine,0, SV2.Top + SV2.Height, cWidth, cStatusLineHeight)
	Else
	...
	If mFastScrollFixedLabel = False Then
		pnlTable.AddView(lblFastScroll,  cWidth - mFastScrollLabelWidth - FScCursorWidth, FscLabelTopDelta, mFastScrollLabelWidth, mFastScrollLabelHeight)
	Else
...
'RBS altered to have rhombus sort icon if all values equal
Private Sub InitializeSortingBitmaps
	...
	'RBS added to have rhombus sort icon if all values equal
...
'draws the fast scroll cursor
Private Sub DrawFastScrollCursor
...
'			w = Max(2dip, Widths(col) - cLineWidth)
	...
	If mFastScrollFixedLabel = True Then
		lblFastScroll.Top = (SV2.Height - lblFastScroll.Height) / 2 + Header.Height
	End If
...
'		txtColor = cTextColor
	...
	Else If cUseColumnColors = True Then
		rowColor = ColumnDrawables
		txtColor = cTextColor
	Else
		If Row Mod 2 = 0 Then
			rowColor = Drawable1
	...
	If mNumberOfFixedColumns = 0 Then
		For i = 0 To lbls.Length - 1
			If (Header.GetView(I).Width > 1 ) Then
				...
				Else
					If MultiTextColors = False Then
						lbls(i).TextColor = txtColor
					Else
						lbls(i).TextColor = cTextColors(i)
					End If
...
'			HeaderFirst.AddView(lbl, Left, 0, w, cHeaderHeight - cLineWidth)
		Else
			If col = mNumberOfFixedColumns Then
				Left = 0
			End If
			Header.AddView(lbl, Left, 0, w, cHeaderHeight)
'			Header.AddView(lbl, Left, 0, w, cHeaderHeight - cLineWidth)
		End If
...
Public Sub LoadTableFromCSV(Dir As String, Filename As String, HeadersExist As Boolean)
	...
	'RBS, added to do with new sorting code
	'--------------------------------------
...
Public Sub LoadTableFromCSV2(Dir As String, Filename As String, HeadersExist As Boolean, SeparatorChar As String, AutomaticWidths As Boolean)
	...
	If HeadersExist Then
		...
		List1 = StringUtils1.LoadCSV2(Dir, Filename, SeparatorChar, lstHeaders)
		Dim Headers(lstHeaders.Size) As String
		For i = 0 To lstHeaders.Size - 1
			Headers(i) = lstHeaders.Get(i)
		Next
	Else
		List1 = StringUtils1.LoadCSV(Dir, Filename, SeparatorChar)
		Dim firstRow() As String
		firstRow = List1.Get(0)
		Dim Headers(firstRow.Length) As String
		For i = 0 To firstRow.Length - 1
			Headers(i) = "Col" & (i + 1)
		Next
	End If
	innerClearAll(Headers.Length, True)
	Dim ColumnWidths(mNumberOfColumns) As Int
	Dim HeaderWidths(mNumberOfColumns) As Int
	Dim DataWidths(mNumberOfColumns) As Int
	Dim cColumnDataType(mNumberOfColumns) As String
	...
	Else
		If HeadersExist Then
			...
			If HeaderMultiTypeFace = False Then
				For col = 0 To mNumberOfColumns - 1
					HeaderWidths(col) = cvs.MeasureStringWidth(lstHeaders.Get(col), cHeaderTypeFace, cTextSize)  + ExtraWidth
				Next
			Else
				For col = 0 To mNumberOfColumns - 1
					HeaderWidths(col) = cvs.MeasureStringWidth(lstHeaders.Get(col), cHeaderTypeFaces(col), cTextSize)  + ExtraWidth
				Next
			End If
		End If
...
Public Sub SaveTableToCSV(Dir As String, Filename As String)
	...
	Else
		For i = 0 To mNumberOfColumns - 1
			Dim L As B4XView
			If i < mNumberOfFixedColumns Then
				L = HeaderFirst.GetView(i)
			Else
				L = Header.GetView(i - mNumberOfFixedColumns)
				Headers(i) = L.Text
			End If
		Next
	End If
...
Public Sub RemoveRow(Row As Int)	
	...
	For i=0 To SelectedRows.Size -1 ' updated selection
		...
		If (keepSel > Row) Then 			
			SelectedRows.set(i,keepSel-1) ' dec row number in all rows appear after the soon to be removed removed row
...
Public Sub insertRowAt (Row As Int, Values() As String) As Boolean
	...
	lstRowColorIndexes.InsertAt(Row, 0)
...
Public Sub RefreshLabels
	...
	If cTypeFaces.Length < mNumberOfColumns Then
		Private cTypeFaces(mNumberOfColumns) As Typeface
		For j = 0 To mNumberOfColumns - 1
			cTypeFaces(j) = cTypeFace
		Next
	End If
	
	If cTextColors.Length < mNumberOfColumns Then
		Private cTextColors(mNumberOfColumns) As Int
		For j = 0 To mNumberOfColumns - 1
			cTextColors(j) = cTextColor
		Next
	End If
	For i = 0 To LabelsCache.Size - 1
		...
		For j = 0 To lbls.Length - 1
			If MultiTypeFace = True Then
				...
				lbls(j).TextColor = cTextColors(j)
			Else
				lbls(j).Typeface = cTypeFace
				lbls(j).TextColor = cTextColor
			End If
		Next
	Next
...
Public Sub setStatusLine(Text As String)
	If (lblStatusLine.IsInitialized) Then lblStatusLine.Text = Text
End Sub
...
Public Sub setHeight(Height As Int)
	...
	If (cShowStatusLine = True) Then
		SVF.Height = Height - cStatusLineHeight - cHeaderHeight
		SV2.Height = Height - cStatusLineHeight - cHeaderHeight
	Else
...
'returns the index index of the first visible row
'read only
Public Sub getTopRowIndex As Int
	Return SV2.VerticalScrollPosition / cRowHeight
End Sub
...
Private Sub Header_Click
	...
	'Log("Header_Click, cSortColumn: " & cSortColumn)
	
	If cSortColumn Then 'this is set in the designer and applies to all columns (?)
...
'	Dim ll As Int = 10dip
	...
	Select Case dir
		...
		Case 1
			...
			t = View.Top + View.Height - cSortBitmapHeight - 2dip
		Case 2 'this was added to show through the header sorting icon that all values were the same
...
'sets the cell alignment for the given column each column can have a diferent alignment
'Example:'<code>
'Table1.SetCellAlignmentColN(Col, Bit.Or(Gravity.LEFT, Gravity.CENTER_VERTICAL))</code>
Public Sub SetCellAlignmentColN(Col As Int, AlignmentColN As Int)
	Private i As Int
	
	Col = Max(0, Col)
	If Col > mNumberOfColumns - 1 Then
		ToastMessageShow("The column index is higher than the number of columns.", False)
		Return
	End If
	...
	If MultiAlignments = False Then
		...
		Next
	End If
	cAlignments(Col) = AlignmentColN
	cAlignments0 = cAlignments
...
Public Sub SetHeaderAlignments(Alignments() As Int)
	...
	If Header.NumberOfViews > 0 Or HeaderFirst.NumberOfViews > 0 Then
		If mNumberOfFixedColumns > 0 Then
			For col = 0 To mNumberOfFixedColumns - 1
...
'sets the header alignment for the given column, each column can have a diferent alignment
'Example:'<code>
'Table1.SetHeaderAlignmentColN(Col, Bit.Or(Gravity.LEFT, Gravity.CENTER_VERTICAL))</code>
Public Sub SetHeaderAlignmentColN(Col As Int, AlignmentColN As Int)
	Private i As Int
	...
	If HeaderMultiAlignments = False Then
		Dim cHeaderAlignments0(mNumberOfColumns) As Int
		For i = 0 To mNumberOfColumns - 1
			cHeaderAlignments0(i) = cHeaderAlignment
		Next
	End If
	cHeaderAlignments0(Col) = AlignmentColN
	cHeaderAlignments = cHeaderAlignments0
	If Header.NumberOfViews > 0 Then
		...
		Else
			For i = 0 To mNumberOfFixedColumns - 1
				...
				Else
					lbl = Header.GetView(Col)
					lbl.Gravity = cHeaderAlignments(Col + mNumberOfFixedColumns)
				End If
			Next
		End If
	End If
...
Private Sub CalcFastScroll(Pos As Int)
	...
	If Data.Size > 0 Then
		...
		str = Data.Get(i)
		lblFastScroll.Text = str(mFastScrollColumnIndex).SubString2(0, Min(str(mFastScrollColumnIndex).Length, mFastScrollLabelMaxChars))
...
Public Sub setHeaderHeight(Height As Int)
	...
	If Header.IsInitialized Then
		...
		If cShowStatusLine = True Then
			SV2.Height = pnlTable.Height - cHeaderHeight - cStatusLineHeight
		Else
...
Public Sub setTextColor(Color As Int)
	...
	Else
		If SV2.IsInitialized = True Then
			Dim i As Int
			For i = 0 To SVF.Panel.NumberOfViews - 1
				Dim lbl As Label
				lbl = SVF.Panel.GetView(i)
				lbl.TextColor = cTextColor
			Next
			Dim i As Int
			For i = 0 To SV2.Panel.NumberOfViews - 1
...
Public Sub setTextSize(Size As Float)
	...
	Else
		If Header.IsInitialized Then
			For i = 0 To HeaderFirst.NumberOfViews - 1
				Dim lbl As Label
...
'sets or gets the status line height
Public Sub setStatusLineHeight(StatusLineHeight As Int)
	cStatusLineHeight = StatusLineHeight
End Sub

Public Sub getStatusLineHeight As Int
	Return cStatusLineHeight
End Sub
...
'		"I" Y integer number
'Example:
'<code>Table1.LoadSQLiteDB2(SQL1, "SELECT * FROM Test", True, Array As String("I", "T", "R", "R"))</code>
Public Sub LoadSQLiteDB2(SQLite As SQL, Query As String, AutomaticWidths As Boolean, ColumnDataTypes() As String)
	...
	Dim dd As Double
	Dim str As String
	cColumnDataType = ColumnDataTypes
	For col = 0 To mNumberOfColumns - 1
		Headers(col) = Curs.GetColumnName(col)
		If AutomaticWidths = False Then
			ColumnWidths(col) = 130dip
...
Public Sub setUseColumnColors(UseColumnColors As Boolean)
	...
	If cHeaderTextColors.Length = 0 Then
		Dim i As Int
...
Public Sub SetColumnColors(ColumnColors() As Int)
	...
	Dim ColumnDrawables(cColumnColors.Length) As Object
	For i = 0 To mNumberOfColumns - 1
		Private cd As ColorDrawable
		cd.Initialize(cColumnColors(i), 0)
		ColumnDrawables(i) = cd
	Next

	If ColumnColors.Length > 1 Then
		cUseColumnColors = True
	Else
		cUseColumnColors = False
	End If
	If Data.Size > 0 Then
		RefreshTable
	End If
...
Public Sub SetTextColors(TextColors() As Int)
	...
	If cTextColors.Length = 1 Then
		MultiTextColors = False
		setTextColor(cTextColors(0))
		Return
	Else
		MultiTextColors = True
	End If
	If mNumberOfFixedColumns = 0 Then
		If SV2.IsInitialized = True Then
			Dim i, col As Int
			For i = 0 To SV2.Panel.NumberOfViews - 1
				col = i Mod mNumberOfColumns
				Dim lbl As Label
				lbl = SV2.Panel.GetView(i)
				lbl.TextColor = cTextColors(col)
			Next
		End If
	Else
		If SV2.IsInitialized = True Then
			Dim i, col As Int
			For i = 0 To SVF.Panel.NumberOfViews - 1
				col = i Mod mNumberOfFixedColumns
				Dim lbl As Label
				lbl = SVF.Panel.GetView(i)
				lbl.TextColor = cTextColors(col)
			Next
			For i = 0 To SV2.Panel.NumberOfViews - 1
				col = i Mod (mNumberOfColumns - mNumberOfFixedColumns) + mNumberOfFixedColumns
				Dim lbl As Label
				lbl = SV2.Panel.GetView(i)
				lbl.TextColor = cTextColors(col)
			Next
...
Public Sub SetAutomaticWidths
	...
	For col = 0 To mNumberOfColumns - 1
		If MultiTypeFace = False Then
			Widths(col) = cvs.MeasureStringWidth(HeaderNames.Get(col), cHeaderTypeFace, cTextSize) + ExtraWidth
		Else
			Widths(col) = cvs.MeasureStringWidth(HeaderNames.Get(col), cHeaderTypeFaces(col), cTextSize) + ExtraWidth
		End If
...
Public Sub setShowStatusLine(ShowStatusLine As Boolean)
	cShowStatusLine = ShowStatusLine
	If cShowStatusLine = True Then
		SV2.Height = cHeight - Header.Height - cStatusLineHeight
...
'		cTextSize = cTextSize * Min(ScaleX, ScaleY)
		...
		If mNumberOfFixedColumns = 0 Then
			SV2.Width = cWidth
		Else
			SVF.Width = SVF.Width * ScaleX
			SV2.Width = SV2.Width * ScaleX
			SV2.Left = SVF.Width
		End If
		SV2.Top = cHeaderHeight
		SVF.Top = cHeaderHeight
		If (cShowStatusLine = True) Then
			SV2.Height = cHeight - cStatusLineHeight - cHeaderHeight
			SVF.Height = cHeight - cStatusLineHeight - cHeaderHeight
		Else
			SV2.Height = cHeight - cHeaderHeight
			SVF.Height = cHeight - cHeaderHeight
		End If
		lblFastScroll.Width = lblFastScroll.Width * ScaleX
		lblFastScroll.Height = lblFastScroll.Height * ScaleY
'		lblFastScroll.TextSize = lblFastScroll.TextSize * Min(ScaleX, ScaleY)
		lblFastScroll.TextSize = lblFastScroll.TextSize * ScaleY
		lblFastScroll.Left = lblFastScroll.Left * ScaleX
		lblFastScroll.Top = (SV2.Height - lblFastScroll.Height) / 2 + Header.Height
		InitFastScroll
		lblStatusLine.TextSize = cTextSize
		lblStatusLine.Height = cStatusLineHeight
		lblStatusLine.Top = SV2.Top + SV2.Height
	End If
	CreateNewLabels
End Sub
...
'DataType can be either "T" for TEXT, "R" for REAL or "I" for INTEGER
Public Sub SetColumnDataType(Column As Int, DataType As String)
	...
	If DataType <> "T" And DataType <> "R" And DataType <> "I" Then
...
'DataType() array, can be either "T" for TEXT, "R" for REAL or "I" for INTEGER'Example:
'<code>SetColumnDataTypes(Array As String("I", "T", "R", "T", "I")</code>
Public Sub SetColumnDataTypes(DataType() As String)
	...
	For Col = 0 To mNumberOfColumns - 1
		If DataType(Col) <> "T" And DataType(Col) <> "R" And DataType(Col) <> "I" Then
			ToastMessageShow("Wrong data type only T, R and I are allowed", False)
			Return		
		End If	
	Next
...
'returns an Array of Strings, the values are either T, R or I
Public Sub GetColumnDataTypes As String()
	Return cColumnDataType
End Sub

'gets or sets the number of columns, read only
Public Sub getNumberOfColumns As Int
	Return mNumberOfColumns
End Sub

Public Sub setNumberOfColumns (NumberOfColumns As Int)
	mNumberOfColumns = NumberOfColumns
	innerClearAll(mNumberOfColumns, True)
...
'gets or sets the FastScrollLabelMaxChars property, default value = 10.
Public Sub getFastScrollLabelMaxChars As Int
	Return mFastScrollLabelMaxChars
End Sub

Public Sub setFastScrollLabelMaxChars(FastScrollLabelMaxChars As Int)
	mFastScrollLabelMaxChars = FastScrollLabelMaxChars
End Sub

'gets or sets the FastScrollLabelWidth property
Public Sub getFastScrollLabelWidth As Int
	Return mFastScrollLabelWidth
End Sub

Public Sub setFastScrollLabelWidth(Width As Int)
	mFastScrollLabelWidth = Width
	lblFastScroll.Width = mFastScrollLabelWidth
	If mFastScrollFixedLabel = False Then
		lblFastScroll.Left = cWidth - mFastScrollLabelWidth - mFastScrollLabelWidth
	Else
		lblFastScroll.Left = 0
...
'gets or sets the FastScrollLabelHeight property
Public Sub getFastScrollLabelHeight As Int
	Return mFastScrollLabelWidth
End Sub

Public Sub setFastScrollLabelHeight(Height As Int)
	mFastScrollLabelHeight = Height
	lblFastScroll.Height = mFastScrollLabelHeight
	If mFastScrollFixedLabel = False Then
		FscLabelTopDelta = SV2.Top + (FScCursorHeight - mFastScrollLabelHeight) / 2
		lblFastScroll.Top = pnlFastScrollCursor.Top + FscLabelTopDelta
	Else
...
'-------------------------------------------
'RBS Added, all the new sorting related code
'-------------------------------------------
Public Sub SortTable2(Col As Int, bAscending As Boolean) As Boolean
	...
	'It is needed to produce the sort arrays
	...
	'-----------------------
	'setup the 1D sort array
	'-----------------------
	If arrDataSorted(Col) = False Then
		Log("SortTable2, cColumnDataType(" & Col & "): " & cColumnDataType(Col))
		Select Case cColumnDataType(Col)
			Case "I", "L"
	...
	'------------------------------------
	bValidArrSortIndex = arrSortIndex.Length > 0 And arrSortIndex(0) > -1
	bDoMultiColumnSort = mMultiColumnSort And bValidArrSortIndex And mNumberOfColumns > 1
	...
	If arrDataSorted(Col) = False Then
		...
		Select Case cColumnDataType(Col)
			Case "I", "L"
				'Log("Table, sorting Int")
				If bDoMultiColumnSort Then
					arrSortIndex = SortOMLongIDX2(arrColLong, arrSortIndex, True, -1, -1)
				Else
					arrSortIndex = SortOMLongIDX(arrColLong, True, -1, -1)
				End If
			Case "R"
				'Log("Table, sorting Real")
				If bDoMultiColumnSort Then
					arrSortIndex = SortOMDoubleIDX2(arrColDouble, arrSortIndex, True, -1, -1)
				Else
					arrSortIndex = SortOMDoubleIDX(arrColDouble, True, -1, -1)
				End If
			Case "T"
				'Log("Table, sorting Text")
				If bDoMultiColumnSort Then
					arrSortIndex = SortOMStringIDX2(arrColString, arrSortIndex, True, mSortCaseInsensitive, -1, -1)
				Else
					arrSortIndex = SortOMStringIDX(arrColString, True, mSortCaseInsensitive, -1, -1)
				End If
			Case "N", "B"'all values are null, do no sort
				'Log("Table, sorting Nil, as data type N or B")
	...
	'this is better than just reversing the Data.Set loop as that doesn't maintain
	...
	If bAscending = False And bValidArrSortIndex Then
		Select Case cColumnDataType(Col)
			Case "I", "L"
				arrSortIndex = ReverseIndexLong(arrColLong, arrSortIndex, False)
			Case "R"
				arrSortIndex = ReverseIndexDouble(arrColDouble, arrSortIndex, False)
			Case "T"
				arrSortIndex = ReverseIndexString(arrColString, arrSortIndex, False)
		End Select
...
Sub SortOMStringIDX2(arrString() As String, arrIndex() As Int, bAscending As Boolean, bCaseInsensitive As Boolean, iStart As Int, iEnd As Int) As Int()
	...
	Dim c As Long
	Dim n As Long
	Dim B4XOM As B4XOrderedMap 'this needs the B4XCollections libarary
...
Public Sub setMultiColumnSort(bDo As Boolean)
	mMultiColumnSort = bDo
End Sub

Public Sub getMultiColumnSort As Boolean
	Return mMultiColumnSort
End Sub

Public Sub setSortCaseInsensitive(bDo As Boolean)
	mSortCaseInsensitive = bDo
End Sub

Public Sub getSortCaseInsensitive As Boolean
	Return mSortCaseInsensitive
End Sub